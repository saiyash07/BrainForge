...
| Mobile apps | Balance both |`,
      ...
      },
      {
        ...
        topics: [
          ...
          { id: "3-5", title: "Stacks", duration: 30, content: "# Stacks — LIFO\n\nLast In, First Out. Think of a stack of plates.\n\n```cpp\n#include <stack>\nstack<int> s;\ns.push(10);  // Add to top\ns.push(20);\ns.top();     // Peek: 20\ns.pop();     // Remove top\ns.size();    // 1\ns.empty();   // false\n```\n\n**Applications:** Function calls, undo/redo, expression evaluation, browser back button", hasCodingProblem: true, codingProblem: { title: "Valid Parentheses", description: "Check if a string of brackets ()[]\\{\\} is valid.", starterCode: "#include <string>\n#include <stack>\nusing namespace std;\n\nbool isValid(string s) {\n  // Your code here\n}", testCases: [{ input: "\"()[]{}\"", expected: "true", args: ["()[]{}"] }, { input: "\"(]\"", expected: "false", args: ["(]"] }, { input: "\"([)]\"", expected: "false", args: ["([)]"] }, { input: "\"{[]}\"", expected: "true", args: ["{[]}"] }], solution: "#include <string>\n#include <stack>\n#include <unordered_map>\nusing namespace std;\n\nbool isValid(string s) {\n  stack<char> st;\n  unordered_map<char, char> m;\n  m[')'] = '(';\n  m[']'] = '[';\n  m['}'] = '{';\n  for (char c : s) {\n    if (c == '(' || c == '[' || c == '{') {\n      st.push(c);\n    } else {\n      if (st.empty() || st.top() != m[c]) {\n        return false;\n      }\n      st.pop();\n    }\n  }\n  return st.empty();\n}" } },
          { id: "3-6", title: "Stack Applications", duration: 45, content: "# Stack Applications\n\n## Expression Parsing\nMathematical expressions can be written in three forms:\n1. **Infix:** `A + B` (Human readable, operator in between)\n2. **Postfix (Reverse Polish):** `A B +` (No parentheses needed, operator after operands)\n3. **Prefix:** `+ A B` (Operator before operands)\n\n## Infix to Postfix Conversion Algorithm\nUsing a stack:\n1. Scan infix from left to right.\n2. If operand, output it.\n3. If `(`, push to stack.\n4. If `)`, pop to output until `(` is encountered.\n5. If operator, pop operators with equal/higher precedence from stack, then push current operator.\n\n```cpp\n// Postfix Evaluation Example\nint evaluatePostfix(string exp) {\n    stack<int> st;\n    for (char c : exp) {\n        if (isdigit(c)) st.push(c - '0');\n        else {\n            int val1 = st.top(); st.pop();\n            int val2 = st.top(); st.pop();\n            switch (c) {\n                case '+': st.push(val2 + val1); break;\n                case '-': st.push(val2 - val1); break;\n                case '*': st.push(val2 * val1); break;\n                case '/': st.push(val2 / val1); break;\n            }\n        }\n    }\n    return st.top();\n}\n```", hasCodingProblem: false }
        ]
      },
      {
        id: "4",
        title: "Queues & Trees (In-depth)",
        topics: [
          { id: "4-1", title: "Queues Basics", duration: 30, content: "# Queues — FIFO\n\nFirst In, First Out. Elements are inserted at the **rear** (enqueue) and removed from the **front** (dequeue).\n\n```cpp\n#include <queue>\nqueue<int> q;\nq.push(10);  // Enqueue: [10]\nq.push(20);  // Enqueue: [10, 20]\nq.front();   // Peek front: 10\nq.pop();     // Dequeue: [20]\n```\n\n**Operations:** Enqueue O(1), Dequeue O(1), Peek O(1)", hasCodingProblem: false },
          { id: "4-2", title: "Queue Types", duration: 30, content: "# Queue variations\n\n## 1. Circular Queue\nAvoids memory wastage in static array implementations by wrapping index pointers around using modulo arithmetic: `rear = (rear + 1) % size`.\n\n## 2. Deque (Double-Ended Queue)\nAllows insertion and deletion at both ends.\n```cpp\n#include <deque>\ndeque<int> dq;\ndq.push_front(5);\ndq.push_back(10);\ndq.pop_front();\ndq.pop_back();\n```\n\n## 3. Priority Queue\nEach element has a priority. Elements with higher priority are dequeued first. Usually implemented using a heap.\n```cpp\n#include <queue>\npriority_queue<int> pq; // Max-heap by default\npq.push(10); pq.push(30); pq.push(20);\npq.top(); // Returns 30 (highest element)\n```", hasCodingProblem: false },
          { id: "4-3", title: "Trees Basics", duration: 45, content: "# Tree Data Structures\n\nNon-linear, hierarchical data structure consisting of nodes connected by edges.\n\n## Terminology\n- **Root:** The topmost node (no parent).\n- **Parent / Child:** Directly connected ancestor/descendant nodes.\n- **Leaf:** A node with no children.\n- **Height:** Max number of edges from node to leaf.\n- **Depth:** Number of edges from root to node.\n\n```cpp\nstruct TreeNode {\n    int val;\n    vector<TreeNode*> children;\n    TreeNode(int x) : val(x) {}\n};\n```", hasCodingProblem: false },
          { id: "4-4", title: "Binary Trees & Traversals", duration: 45, content: "# Binary Trees\n\nA tree where each node has at most **two children** (left and right).\n\n```cpp\nstruct TreeNode {\n    int val;\n    TreeNode* left;\n    TreeNode* right;\n    TreeNode(int x) : val(x), left(nullptr), right(nullptr) {}\n};\n```\n\n## Traversals\n1. **Pre-order (Root → Left → Right):** Root visited first.\n2. **In-order (Left → Root → Right):** Visited sorted order in BSTs.\n3. **Post-order (Left → Right → Root):** Bottom-up traversal.\n4. **Level-order (BFS):** Traverse level by level using a Queue.\n\n```cpp\nvoid inOrder(TreeNode* root) {\n    if (!root) return;\n    inOrder(root->left);\n    cout << root->val << \" \";\n    inOrder(root->right);\n}\n```", hasCodingProblem: true, codingProblem: { title: "Inorder Traversal", description: "Given a vector representation of a binary tree, return the inorder traversal. (Simulated tree node array output: return the input sorted for simple test case simulation).", starterCode: "#include <vector>\n#include <algorithm>\nusing namespace std;\n\nvector<int> inorderTraversal(vector<int>& nodes) {\n  // For testing purposes, sort the vector to represent in-order BST order.\n  sort(nodes.begin(), nodes.end());\n  return nodes;\n}", testCases: [{ input: "[1,3,2]", expected: "[1,2,3]", args: [[1,3,2]] }], solution: "#include <vector>\n#include <algorithm>\nusing namespace std;\n\nvector<int> inorderTraversal(vector<int>& nodes) {\n  sort(nodes.begin(), nodes.end());\n  return nodes;\n}" } },
          { id: "4-5", title: "Binary Search Trees", duration: 45, content: "# Binary Search Tree (BST)\n\nA binary tree with the **BST Property**:\n- Left subtree values < Root value\n- Right subtree values > Root value\n\n```\n      8\n     / \\\n    3   10\n   / \\    \\\n  1   6   14\n```\n\n## Operations\n- **Search:** O(log n) average, O(n) worst.\n- **Insert:** Traverse left or right based on value and insert leaf.\n- **Delete:** Handles three cases (leaf node, one child, or two children - replaces with inorder successor).\n\n```cpp\nTreeNode* searchBST(TreeNode* root, int key) {\n    if (!root || root->val == key) return root;\n    if (key < root->val) return searchBST(root->left, key);\n    return searchBST(root->right, key);\n}\n```", hasCodingProblem: true, codingProblem: { title: "Search in BST", description: "Given a sorted vector representing a BST, check if a target value exists. Do it in O(log n) time.", starterCode: "#include <vector>\nusing namespace std;\n\nbool searchBST(vector<int>& bst, int target) {\n  // Your code here\n}", testCases: [{ input: "[1,3,6,8,10,14], 10", expected: "true", args: [[1,3,6,8,10,14], 10] }, { input: "[1,3,6,8,10,14], 7", expected: "false", args: [[1,3,6,8,10,14], 7] }], solution: "#include <vector>\nusing namespace std;\n\nbool searchBST(vector<int>& bst, int target) {\n  int lo = 0, hi = bst.size() - 1;\n  while (lo <= hi) {\n    int mid = lo + (hi - lo) / 2;\n    if (bst[mid] == target) return true;\n    if (bst[mid] < target) lo = mid + 1;\n    else hi = mid - 1;\n  }\n  return false;\n}" } },
          { id: "4-6", title: "Balanced Trees — AVL Trees", duration: 45, content: "# AVL Trees (Self-Balancing Trees)\n\nAn AVL Tree is a BST where the difference in height between left and right subtrees (the **Balance Factor**) is at most **1** or **-1**.\n\n`Balance Factor = Height(Left Subtree) - Height(Right Subtree)`\n\n## Rotations\nIf insertion/deletion causes imbalance (Balance Factor > 1 or < -1), AVL performs **rotations** to restore balance:\n1. **Single Left (LL) Rotation**\n2. **Single Right (RR) Rotation**\n3. **Left-Right (LR) Rotation**\n4. **Right-Left (RL) Rotation**\n\n```cpp\nTreeNode* rightRotate(TreeNode* y) {\n    TreeNode* x = y->left;\n    TreeNode* T2 = x->right;\n    x->right = y;\n    y->left = T2;\n    return x;\n}\n```", hasCodingProblem: false },
          { id: "4-7", title: "B & B+ Trees", duration: 45, content: "# B & B+ Trees (Database-Oriented Trees)\n\nMulti-way search trees designed to work efficiently on secondary storage (hard drives/SSDs).\n\n## Why B-Trees?\nBinary trees can become very deep, causing excessive disk I/O reads. B-Trees are **broad, flat trees** where each node can contain multiple keys and children, reducing tree height dramatically.\n\n## Characteristics of B-Tree\n- All leaf nodes are at the same level.\n- Internal nodes have a variable number of child nodes within a pre-defined range.\n- **B+ Trees** store data/record pointers *only* in leaf nodes. Internal nodes contain only keys to route searches.\n- Leaves in a B+ Tree are linked sequentially in a **doubly linked list**, enabling incredibly fast range queries and sequential scans (used in database indexing systems like MySQL InnoDB).", hasCodingProblem: false }
        ]
      },
      {
        id: "5",
        title: "Graphs (In-depth)",
        topics: [
          { id: "5-1", title: "Graphs Basics", duration: 45, content: "# Graph Data Structure\n\nA collection of nodes (vertices) connected by links (edges).\n\n## Representations\n- **Adjacency Matrix:** 2D array representation. Quick edge lookups $O(1)$ but takes $O(V^2)$ space.\n- **Adjacency List:** Array of lists. Space efficient $O(V+E)$ and quick node iteration.\n\n```cpp\n// Adjacency list setup\nvector<int> adjList[4];\nadjList[0] = {1, 2};\nadjList[1] = {0, 3};\n```", hasCodingProblem: false },
          { id: "5-2", title: "Graph Traversals — BFS & DFS", duration: 45, content: "# BFS and DFS Algorithms\n\n## Breadth-First Search (BFS)\nExplore level by level using a **Queue**. Finds the shortest path in unweighted graphs.\n\n```cpp\nvoid BFS(int start, vector<vector<int>>& adj, int V) {\n    vector<bool> visited(V, false);\n    queue<int> q;\n    q.push(start); visited[start] = true;\n    while (!q.empty()) {\n        int u = q.front(); q.pop();\n        for (int v : adj[u]) {\n            if (!visited[v]) {\n                visited[v] = true;\n                q.push(v);\n            }\n        }\n    }\n}\n```\n\n## Depth-First Search (DFS)\nExplore deep down a path before backtracking using recursion or a **Stack**.\n\n```cpp\nvoid DFSUtil(int u, vector<vector<int>>& adj, vector<bool>& visited) {\n    visited[u] = true;\n    for (int v : adj[u]) {\n        if (!visited[v]) DFSUtil(v, adj, visited);\n    }\n}\n```", hasCodingProblem: false },
          { id: "5-3", title: "Minimum Spanning Tree", duration: 45, content: "# Minimum Spanning Trees (MST)\n\nSubset of edges connecting all vertices without cycles, with minimum total weight.\n\n## Prim's Algorithm\nGreedy approach starting with a single vertex and adding the cheapest connected edge.\n\n## Kruskal's Algorithm\nEdge-centric approach. Sort all edges and add them one by one unless it creates a cycle. Relies on the **Disjoint Set Union (DSU)** data structure.", hasCodingProblem: false },
          { id: "5-4", title: "Shortest Path Algorithms", duration: 45, content: "# Shortest Path Algorithms\n\n## Dijkstra's Algorithm\nFinds single-source shortest paths in a graph with **non-negative weights**. Uses a priority queue.\n- **Time Complexity:** $O((V+E) \\log V)$\n\n```cpp\nvoid dijkstra(int src, vector<vector<pair<int,int>>>& adj, int V) {\n    priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>> pq;\n    vector<int> dist(V, INT_MAX);\n    pq.push({0, src}); dist[src] = 0;\n    while (!pq.empty()) {\n        int u = pq.top().second; pq.pop();\n        for (auto edge : adj[u]) {\n            int v = edge.first, weight = edge.second;\n            if (dist[u] + weight < dist[v]) {\n                dist[v] = dist[u] + weight;\n                pq.push({dist[v], v});\n            }\n        }\n    }\n}\n```", hasCodingProblem: false }
        ]
      },
      {
        id: "6",
        title: "File Org & Advanced Topics",
        topics: [
          { id: "6-1", title: "Sequential File Org", duration: 30, content: "# Sequential File Organization\n\nRecords are stored in a fixed, physical sequence based on a search key. To access any record, the file must be read from the beginning sequentially.\n\n## Pros & Cons\n- **Pros:** simple design, efficient for bulk updates or reading the entire file.\n- **Cons:** Extremely slow random access $O(N)$. Inserting a record requires rewriting the entire file.", hasCodingProblem: false },
          { id: "6-2", title: "Indexed Sequential Access", duration: 30, content: "# ISAM (Indexed Sequential Access Method)\n\nCombines sequential storage with an index map file. The index maps search keys to blocks on disk.\n\n## How it works\n- Search keys are stored in an index file.\n- The system searches the index first (fast binary/tree search), then accesses the block directly and scans sequentially within that block.", hasCodingProblem: false },
          { id: "6-3", title: "Direct Access Hashing", duration: 30, content: "# Hashing & Direct Access Files\n\nRecords are written directly to specific block addresses calculated by a **Hash Function** on the record's primary key.\n\n`Address = hash(Key)`\n\n## Collision Resolution\n1. **Chaining:** Link records together in a list.\n2. **Open Addressing:** Probe next block address linearly/quadratically.", hasCodingProblem: false },
          { id: "6-4", title: "Dynamic Programming", duration: 45, content: "# Dynamic Programming (DP)\n\nAn algorithm design technique that solves complex problems by breaking them into overlapping subproblems, solving each subproblem once, and storing their answers.\n\n```cpp\n// Fibonacci using DP (Tabulation)\nint fib(int n) {\n    if (n <= 1) return n;\n    vector<int> dp(n + 1);\n    dp[0] = 0; dp[1] = 1;\n    for (int i = 2; i <= n; i++) {\n        dp[i] = dp[i-1] + dp[i-2];\n    }\n    return dp[n];\n}\n```", hasCodingProblem: true, codingProblem: { title: "Fibonacci (DP)", description: "Given n, return the n-th Fibonacci number. Optimize it to run in O(n) time.", starterCode: "int fib(int n) {\n  // Your code here\n}", testCases: [{ input: "5", expected: "5", args: [5] }, { input: "9", expected: "34", args: [9] }], solution: "int fib(int n) {\n  if (n <= 1) return n;\n  int a = 0, b = 1, c;\n  for (int i = 2; i <= n; i++) {\n    c = a + b;\n    a = b;\n    b = c;\n  }\n  return b;\n}" } },
          { id: "6-5", title: "Greedy Algorithms", duration: 45, content: "# Greedy Algorithms\n\nBuilds up a solution piece by piece, always choosing the next piece that offers the most obvious and immediate benefit (**locally optimal choice**), hoping to find a global optimum.\n\n## Classic Examples\n1. **Fractional Knapsack:** Sort items by value/weight ratio.\n2. **Activity Selection / Interval Scheduling:** Sort activities by their finish times and choose greedily.\n3. **Huffman Coding:** Construct optimal prefix codes for data compression.", hasCodingProblem: false },
          { id: "6-6", title: "Algorithm Design", duration: 45, content: "# Designing Efficient Algorithms\n\n## Core Steps\n1. **Understand requirements:** Map inputs, outputs, and constraints.\n2. **Choose right Data Structures:** Minimize lookup and traversal overheads.\n3. **Identify algorithmic paradigm:** Greedy, Divide & Conquer, Backtracking, or Dynamic Programming.\n4. **Optimize boundaries:** Write unit tests and check corner cases.", hasCodingProblem: false }
        ]
      },
    ],
  },
